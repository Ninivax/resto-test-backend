...
/**
 * Carga pdfjs-dist (build legacy, apta para Node) de forma perezosa.
 */
let pdfjs = null;
async function ensurePdfjs() {
  if (!pdfjs) {
    // Usa la build legacy para entornos Node
    pdfjs = await import("pdfjs-dist/legacy/build/pdf.mjs");
    // No necesitamos configurar workerSrc si desactivamos el worker al abrir el doc
  }
  return pdfjs;
}
...
async function extractTextFromPdfBuffer(uint8) {
  ...
  // Desactivamos el worker para evitar el error y problemas de ruta
  const loadingTask = _pdfjs.getDocument({
    data: uint8,              // Uint8Array del PDF
    disableWorker: true,      // 👈 importante en Node
    disableFontFace: true,    // evita cargar fuentes en server
